...
export default function PredefinedTasksScreen() {
  const { user } = useAuth();
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      showAlert('Error', 'Failed to load predefined tasks: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddSubmit = async () => {
    const trimmedName = taskNameForm.trim();
    if (!trimmedName) {
      showAlert('Input Required', 'Please enter a task name.');
      return;
    }
    ...
    try {
      const { error } = await supabase.from('predefined_tasks').insert([
        {
      ...
      if (error) {
        if (error.code === '23505') {
          throw new Error('This task name already exists in your department.');
        }
        throw error;
      }

      showAlert('Success', 'Predefined task added successfully.');
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      showAlert('Success', 'Predefined task updated successfully.');
  ...
  const handleDelete = (task: PredefinedTask) => {
    showAlert(
      'Confirm Delete',
      `Are you sure you want to delete the task "${task.task_name}"?\nThis won't affect past logged task descriptions but will remove it from the dropdown.`,
      [
          ...
          onPress: async () => {
            try {
              const { error } = await supabase
                ...
                .eq('id', task.id);
              if (error) throw error;
              showAlert('Success', 'Predefined task deleted successfully.');
              fetchTasks();
            } catch (err: any) {
              showAlert('Error', 'Failed to delete task: ' + err.message);
            }
          },
        },
      ]
  ...
  const handleOpenEdit = (task: PredefinedTask) => {
    setSelectedTask(task);
    setTaskNameForm(task.task_name);
    ...
    <View style={[styles.container, { paddingTop: insets.top + 12 }]}>
      <View style={[styles.header, { flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center', gap: isMobile ? 12 : 0 }]}>
        <View style={{ flex: isMobile ? undefined : 1 }}>
          <Text style={styles.title}>Predefined Tasks</Text>
          <Text style={styles.subtitle}>Manage standard tasks for {user?.department} employees</Text>
        </View>
        <TouchableOpacity style={[styles.addButton, isMobile && { width: '100%', justifyContent: 'center' }]} onPress={handleOpenAdd}>
          <Ionicons name="add-circle-outline" size={18} color="#FFF" />
          <Text style={styles.addBtnText}>Add Predefined Task</Text>
        </TouchableOpacity>
      </View>
          ...
          {filteredTasks.length === 0 ? (
            <View style={styles.centerEmpty}>
              ...
              <Text style={styles.emptyText}>
                {searchQuery ? 'Try modifying your search term' : 'Predefined tasks will appear as a dropdown in the employee form'}
              </Text>
            </View>
          ) : (
            filteredTasks.map((item) => (
              <View key={item.id} style={styles.card}>
                <View style={styles.cardLeft}>
                  <Ionicons name="checkbox-outline" size={20} color={Brand.colors.primary} style={{ marginRight: 12 }} />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Add Predefined Task</Text>
            ...
            <View style={styles.form}>
              <View style={styles.formGroup}>
                <Text style={styles.label}>Task Name / Description</Text>
                <TextInput
                  style={styles.modalInput}
                  placeholder="e.g. Coil Loading & Verification"
                  value={taskNameForm}
                  onChangeText={setTaskNameForm}
                  autoFocus
                />
              </View>
      ...
      <Modal visible={isEditModalOpen} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Edit Predefined Task</Text>
