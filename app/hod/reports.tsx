...
export default function ReportsScreen() {
  ...
  const loadJsPDFLibrary = (): Promise<any> => {
    return new Promise((resolve, reject) => {
      ...
      const ttfUrl = "https://cdn.jsdelivr.net/gh/google/fonts@main/ofl/tirotamil/TiroTamil-Regular.ttf";
      ...
      const loadFontAndResolve = async () => {
        try {
          ...
          console.warn("Could not load Tiro Tamil bilingual font, falling back to default Helvetica.", err);
        }
        resolve((window as any).jspdf);
      };
  ...
  const generatePDFBase64 = (jspdfModule: any): string => {
    ...
    if (tamilFontBase64) {
      doc.addFileToVFS("TiroTamil.ttf", tamilFontBase64);
      doc.addFont("TiroTamil.ttf", "TiroTamil", "normal");
      doc.setFont("TiroTamil", "normal");
    }
    ...
    doc.setFont(tamilFontBase64 ? "TiroTamil" : "helvetica", "bold");
    doc.text(labelDept, 14, 40);
    const wDept = doc.getTextWidth(labelDept);
    doc.setFont(tamilFontBase64 ? "TiroTamil" : "helvetica", "normal");
    ...
    if (sortedBacklog.length > 0 && filter !== 'custom') {
      ...
      (doc as any).autoTable({
        ...
        headStyles: { 
          ...
          font: tamilFontBase64 ? 'TiroTamil' : 'helvetica',
          halign: 'left',
          fontSize: 11
        },
        bodyStyles: { 
